    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->comboDevice->addItem(device.description());//添加默认输入设备
    //首选音频格式
    ...
    ui->labelChanCountRange->setText(QString::asprintf("范围:%d~%d",minChan,maxChan));
...
void MainWindow::initChart()
{
    ...
    chart->setTitle("音频输入信号");
    ui->chartView->setChart(chart);
    lineSeries = new QLineSeries();
    chart->addSeries(lineSeries);
    lineSeries->setUseOpenGL(true);//使用 OpenGL 加速

    QValueAxis* axisX = new QValueAxis;
    axisX->setRange(0,m_bufferSize);//设置X轴数据范围
    axisX->setLabelFormat("%g");// %g ?
    QValueAxis* axisY = new QValueAxis;
    axisY->setRange(0,256);  //UInt8采样，数据范围为 0~255
    axisY->setTitleText("Audio Level");

    chart->addAxis(axisX,Qt::AlignBottom);
    chart->addAxis(axisY,Qt::AlignLeft);
    lineSeries->attachAxis(axisX); //将 x 轴依附到序列上
...
void MainWindow::on_actPreferFormat_triggered()
{
    ...
    ui->spinBytesPerSamp->setValue(audioFormat.bytesPerSample());//每采样点的字节数
    ui->spinBytesPerFrame->setValue(audioFormat.bytesPerFrame());//每帧字节数
}
...
void MainWindow::on_actTest_triggered()
{
    ...
    daqFormat.setSampleRate(ui->spinSampRate->value());
    daqFormat.setChannelCount(ui->spinChanCount->value());
    ...
    if(device.isFormatSupported(daqFormat)){
        QMessageBox::information(this,"提示","默认设备支持此格式");
    }else{
        QMessageBox::critical(this,"提示","默认设备不支持此格式");
    }
}
...
void MainWindow::on_spinChanCount_valueChanged(int arg1)
{
    ...
    if(!canShowWave){
        ui->chkBoxShowWave->setEnabled(false);
    }
}
...
void MainWindow::on_actStart_triggered()
{
    if(ui->comboSampFormat->currentIndex()==0){
        QMessageBox::critical(this,"提示","请设置采样点格式");
        return;
    }
    bool saveToFile = ui->chkBoxSaveToFile->isChecked();
    QString fileName = ui->editFileName->text().trimmed();
    if((saveToFile)&&(fileName.isNull())){
        QMessageBox::critical(this,"提示","请设置要保存的文件");
        return;
    }
...
void MainWindow::do_IODevice_update(qint64 blockSize){
    float time = audioSource->processedUSecs()/1000; //毫秒
    QString str = QString::asprintf("已录制时间 =%.1f 秒",time/1000);
    ui->labBufferSize->setText(str);
    ui->labBlockSize->setText(QString("实际数据字节数=%l").arg(blockSize));
}
...
//与  audioSink 的 stateChange() 信号关联
